...
int main() {
   ...
   srand (time(NULL));
   const int TAILLE_TABLEAU = 1000;
   vector<int> tab;
   vector<int> tabResultat(TAILLE_TABLEAU);
   int nbIte = 10;
   for (int i = 1; i <= 100000; i*= 10){
      for (int j = 1; j <= 100000; j*=10){
         cout << "Nombre de donnée à générer : " << i << "\t\tmaxVal : " << j 
                 << "\tTemps moyen (" << nbIte << ") : " << testRandom2(nbIte, i, j) << endl;
      }
   }
   //testrand2
   /*for (int valMax = 1; valMax <= 10; valMax++){
      for (int n = 0; n < 100; n++){
        cout << testRandom2(100, n, 100) << endl;
      }
   }*/
...
//   for (int i = 0; i < 10; i++){
//      f(i, cpt);
//      cout << "Pour n = " << i << " : " << cpt << " et cpt/3 = " << cpt/3.0 << endl;
//      cpt = 0;
//   }
